...
class ContactController {
    ...
    public function index() {
        // Récupére les messages stockés en session (s'ils existent)
        $successMessage = $_SESSION['success_message'] ?? null;
        $errors = $_SESSION['errors'] ?? null;
        
        // Nettoye la session pour que les messages ne s'affichent qu'une seule fois
        unset($_SESSION['success_message'], $_SESSION['errors']);

        // Appele la méthode render avec les données
        $this->render('contact', [
            'pageTitle' => "Contact",
            'currentPage' => "contact",
    ...
    public function submit() {
        // Sécurité : vérifie que la requête est bien de type POST
        ...
        // Si des erreurs sont trouvées, les stocke en session et redirige vers le formulaire
        if (!empty($errors)) {
        ...
        // Si tout est valide, sauvegarde le message via le modèle
        $messageModel = new MessageModel();
        $success = $messageModel->saveMessage($name, $surname, $email, $subject, $message);

        // Stocke un message de succès ou d'échec en session
        if ($success) {
        ...
        // Redirige vers la page de contact pour afficher le message (pattern Post-Redirect-Get)
        header('Location: ' . $this->baseUrl . '/index.php?page=contact');
        exit;
    }
